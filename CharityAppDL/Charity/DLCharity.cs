...
namespace CharityAppDL.Charity
{
    public class DLCharity : DLBase, IDLCharity
    {
        ...
        public CharityVerify? GetVerifiedImage(int charityId)
        {
            ...
            try
            {
                string query = "SELECT c.Id as CharityId, c.CharityFile, cpv.MessageToAdmin, c.IsVerified FROM charities c JOIN charity_process_verify cpv ON c.Id = cpv.CharityId WHERE c.Id = @charityId AND c.IsVerified <> 0 limit 1;";
                DynamicParameters dynamicParameters = new DynamicParameters();
